...
#pragma endregion
	...
	void Update_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		...
		if (registry.ctx().contains<GAME::GameState>())
		{
			auto& gs = registry.ctx().get<GAME::GameState>();
			if (gs.state == GAME::State::MainMenu) {
				vulkanRenderer.clrAndDepth[0].color = { 0.05f, 0.05f, 0.08f, 1.0f }; // dark menu
			}
			else if (gs.state == GAME::State::GameOver) {
				vulkanRenderer.clrAndDepth[0].color = { 0.15f, 0.02f, 0.02f, 1.0f }; // dark red
			}
		}
		...
		if (ui.initialized && !ui.fontsUploaded)
		{
			ui.fontsUploaded = true;
		}
		...
		std::map<GeometryData, int> drawInstructions;
		std::vector<GPUInstance> bufferList;

		entt::entity playerEnt = entt::null;
		GW::MATH::GMATRIXF* playerMatrixPtr = nullptr;

		auto playerView = registry.view<GAME::Player, GAME::Transform>();
		auto pvIt = playerView.begin();
		if (pvIt != playerView.end())
		{
			playerEnt = *pvIt;
			playerMatrixPtr = &registry.get<GAME::Transform>(playerEnt).matrix;
		}

		for (auto ent : instances)
		{
			GPUInstance inst = registry.get<GPUInstance>(ent);

			bool tintRed = false;

			if (playerEnt != entt::null && playerMatrixPtr)
			{
				bool isPlayerMesh = true;
				for (int i = 0; i < 16; ++i)
				{
					if (inst.transform.data[i] != playerMatrixPtr->data[i]) { isPlayerMesh = false; break; }
				}

				if (isPlayerMesh && registry.any_of<GAME::Invulnerability>(playerEnt))
				{
					auto& inv = registry.get<GAME::Invulnerability>(playerEnt);

					int flicker = (int)(inv.cooldown * 40.0f) % 2;
					tintRed = (flicker != 0);
				}
			}
			...
			if (tintRed)
			{
				inst.matData.Kd = { 1.0f, 0.1f, 0.1f };
				inst.matData.Ke = { 0.25f, 0.0f, 0.0f };
			}

			bufferList.push_back(inst);
			drawInstructions[registry.get<GeometryData>(ent)]++;
		}

		registry.emplace_or_replace<std::vector<GPUInstance>>(entity, std::move(bufferList));
		...
		if (registry.all_of< VulkanVertexBuffer, VulkanIndexBuffer>(entity))
		{
			...
			registry.patch<DRAW::VulkanUniformBuffer>(entity);
			...
			for (auto draw : drawInstructions) 
			{
				vkCmdDrawIndexed(commandBuffer, draw.first.indexCount, draw.second, 
					draw.first.indexStart, draw.first.vertexStart, instanceCount);
		...
		if (registry.ctx().contains<GAME::ImGuiState>()) {
			...
			if (imgui.initialized) {
				if (imgui.drawThisFrame) {
					ImGui_ImplVulkan_RenderDrawData(ImGui::GetDrawData(), commandBuffer);
					imgui.drawThisFrame = false;
				}
			}
		}
